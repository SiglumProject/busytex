...
#URL_texlive_full_iso = http://mirrors.ctan.org/systems/texlive/Images/texlive2025-20250308.iso
# Cache not yet uploaded - will need to split and upload the TL2025 ISO to GitHub releases
#URL_texlive_full_iso_cache = (not yet set up for TL2025)
URL_texlive_full_iso = https://tug.ctan.org/systems/texlive/Images/texlive2025-20250308.iso
URL_texlive          = https://github.com/TeX-Live/texlive-source/archive/refs/heads/tags/texlive-2025.2.tar.gz
...
# Note: Removed --unresolved-symbols=ignore-all - it was stubbing critical syscalls
OPTS_BUSYTEX_LINK_PDFTEX_wasm = $(OPTS_BUSYTEX_LINK) -Wl,-error-limit=0 -sINITIAL_MEMORY=$(INITIAL_MEMORY) -sMAXIMUM_MEMORY=$(MAXIMUM_MEMORY) -sSTACK_SIZE=5242880 -sALLOW_MEMORY_GROWTH=1 -sEXIT_RUNTIME=0 -sINVOKE_RUN=0 -sASSERTIONS=1 -sERROR_ON_UNDEFINED_SYMBOLS=1 -sFORCE_FILESYSTEM=1 -sLZ4=1 -sMODULARIZE=1 -sEXPORT_NAME=pdftex -sEXPORTED_FUNCTIONS='["_main", "_flush_streams"]' -sEXPORTED_RUNTIME_METHODS='["callMain", "FS", "ENV", "LZ4", "PATH"]'

build/wasm/pdftex_stubs.o:
	mkdir -p $(dir $@)
	$(CC_wasm) -o $@ -c pdftex_stubs.c $(CFLAGS_OPT_wasm)

build/wasm/pdftex.js: build/wasm/pdftex_stubs.o
	mkdir -p $(dir $@)
	$(CC_wasm) -o build/wasm/pdftex.o -c busytex.c -DBUSYTEX_PDFTEX $(CFLAGS_OPT_wasm)
	$(CXX_wasm) -o $@ build/wasm/pdftex.o build/wasm/pdftex_stubs.o $(addprefix build/wasm/texlive/texk/web2c/, $(OBJ_PDFTEX)) $(addprefix build/wasm/, $(OBJ_DEPS_PDFTEX)) $(addprefix build/wasm/texlive/texk/kpathsea/, $(OBJ_KPATHSEA)) $(OPTS_BUSYTEX_LINK_PDFTEX_wasm)
...
# Minimal dependencies for pdfTeX only (no harfbuzz, icu, fontconfig, etc.)
.PHONY: build/wasm/texlivedependencies-pdftex
build/wasm/texlivedependencies-pdftex:
...
# Minimal applets for pdfTeX only
.PHONY: build/wasm/busytexapplets-pdftex
build/wasm/busytexapplets-pdftex:
...
wasm-pdftex:
	$(MAKE) build/wasm/texlive.configured
	$(MAKE) build/wasm/texlivedependencies-pdftex
	$(MAKE) build/wasm/busytexapplets-pdftex
